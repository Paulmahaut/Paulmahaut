Hi, I'm Paul 👋

🎓 Engineering student in Computer Science  
💻 Interested in Software Engineering and backend-oriented development

## Technical interests
- Backend development and application logic
- Data modeling and relational databases
- Software design fundamentals (architecture, UML, clean code)
- APIs and system interaction

## Technologies I work with
- Programming: Python, Java
- Web fundamentals: HTML, CSS, JavaScript
- Databases: SQL
- Tools & concepts: Git, basic UML, software architecture concepts

## Current focus
- Strengthening software engineering fundamentals
- Building structured personal projects
- Improving code quality, testing, and project organization




<!---
Paulmahaut/Paulmahaut is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
- 💞️ I’m looking to collaborate on ...
- ⚡ Fun fact: ...


- 👋 Hi, I’m @Paulmahaut
- 👀 I’m interested in Trading card games, videogames, in reading fantastic and many others
- 🌱 I’m currently learning Java, html/css/js

- 📫 How to reach me : Discord : paulmahaut
- 😄 Pronouns: He/his
--->
